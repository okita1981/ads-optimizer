          ...
          {[
            {
              ...
              concept: "思考のショートカット",
              desc: "他社と比較せず、迷わず選ぶ状態。GA4の流入データで「どうやって来たか」を見ることで測定できる。",
              signals: [
                "指名検索流入：GA4でsource=googleかつキーワードがブランド名の流入数",
                "直接流入：GA4でsource/medium=directの流入率（URL直打ち・ブックマーク）",
                "比較回避：自社サイト内の比較コンテンツへの遷移率の低さ",
                "休眠復帰：CRMで一定期間購入がなかった後、広告なしで自発的に戻った割合",
              ],
              source: "GA4 + CRM",
              threshold: "< 40 で要改善"
            },
            {
              ...
              concept: "人格の一部化",
              desc: "ブランドが顧客の自己表現の一部になっている状態。POC段階ではCRMの行動データで代替指標として測定する。",
              signals: [
                "高単価プラン・上位オプションへの自発的アップグレード履歴（CRM）",
                "オプション機能の積極利用数・利用率（CRM）",
                "※理想形：レビュー・アンケートの「自分らしい」「ここじゃないとダメ」等の自己表現語彙（言語解析）",
                "※理想形：SNSでのブランド言及・擁護行動（ソーシャルリスニング）",
              ],
              source: "CRM（代替指標）",
              threshold: "< 40 で要改善"
            },
            {
              ...
              concept: "人生への浸透",
              desc: "ブランドが顧客の人生の記憶と結びついている状態。CRMの時間軸ログで「生活に組み込まれているか」を測定できる。",
              signals: [
                "継続年数：CRMの初回購入日から現在までの経過年数",
                "購入周期の規則性：毎年同じ時期に必ず購入する等、CRMの購買ログから算出",
                "ライフイベント接続：住所変更・登録情報更新後も継続利用しているか（CRM）",
                "※理想形：レビュー・アンケートの「初めて使ったのは〜」「あの時助かった」等のエピソード語彙（言語解析）",
              ],
              source: "CRM",
              threshold: "< 30 で要改善"
            },
